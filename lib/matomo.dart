...
class MatomoTracker {
    ...
    initialize({
        ...
        if (kIsWeb) {
            ...
            try {
                ...
                ;
                userAgent = await createDesktopUA();
            }
        }
    ...
    /// ua 格式:
    /// Darwin/<darwin version> (<platform>; <OS> <OS version>) MatomoTrackerSDK/<matomo version> <app name>/<app version>
    Future<String> createDesktopUA() async {
        if (Platform.isMacOS) {
            var info = await DeviceInfoPlugin().macOsInfo;
            var ua = "Windows/20.5.0 (Windows NT 6.1; WOW64) MatomoTrackerSDK/7.3 macOSExampleApp/1.0";//"Darwin/${info.osRelease} (${info.arch}; ${info.model} <OS version>) MatomoTrackerSDK/<matomo version> <app name>/<app version>";
            print("系统信息:$info");
            return ua;
        }
